...
public class aiBoss3 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            var hlp = Quaternion.LookRotation(gameData.playerPosition - transform.position, Vector3.up);
            if (smallCanon.health <= 0)
            {
                Debug.LogError(180+hlp.eulerAngles.y + " " + smallCanon.gameObject.transform.localEulerAngles.y);
                if (hlp.eulerAngles.y > 180 && - 180 + hlp.eulerAngles.y > smallCanon.gameObject.transform.localEulerAngles.y)
                    smallCanon.gameObject.transform.Rotate(new Vector3(0, 0, 1), 20 * Time.deltaTime, Space.Self);
                else if (hlp.eulerAngles.y < 180 && 180 + hlp.eulerAngles.y < smallCanon.gameObject.transform.localEulerAngles.y)
                    smallCanon.gameObject.transform.Rotate(new Vector3(0, 0, 1), -20 * Time.deltaTime, Space.Self);
